...
export async function validateBusinessDateAsync(sql, value) {
  ...
  const exceptions = await sql\`
    ...
    WHERE data = \${dateKey} OR (recorrente_anual = 1 AND right(data, 5) = \${dateKey.slice(5)})
  ...
  const intervals = await sql\`SELECT * FROM intervalos_indisponiveis WHERE dia_semana = \${dayOfWeek} AND ativo = 1\`;
...
export async function hasConflictAsync(sql, startValue, serviceId, ignoreId = null) {
  const services = await sql\`SELECT tempo_estimado FROM servicos WHERE id = \${serviceId} AND ativo = 1\`;
